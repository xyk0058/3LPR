...
class Trainer(object):
    ...
    def get_train_features(self):
        self.model.eval()
        train_labeled_loader = torch.utils.data.DataLoader(self.train_loader.labeled_dataset, batch_size=args.batch_size, shuffle=True, **kwargs)
        self.train_feats = torch.zeros((len(train_labeled_loader.dataset), self.args.proj_size))

        if self.args.dataset == 'cifar10' or self.args.dataset == 'imblancecifar10':
            mean = [0.4914, 0.4822, 0.4465]
            std = [0.2023, 0.1994, 0.2010]
        elif self.args.dataset == 'cifar100' or self.args.dataset == 'imblancecifar100':
            mean = [0.5071, 0.4867, 0.4408]
            std = [0.2675, 0.2565, 0.2761]
        elif self.args.dataset == 'miniimagenet':
            mean = [0.4728, 0.4487, 0.4031]
            std = [0.2744, 0.2663 , 0.2806]
        elif self.args.dataset == 'imblancestl10':
            mean = [0.4914, 0.4822, 0.4465]
            std = [0.2023, 0.1994, 0.2010]
        ...
        if self.args.dataset == 'miniimagenet':
            size = 84
        original_transforms = copy.deepcopy(train_labeled_loader.dataset.transform)
        ...
        with torch.no_grad():
            tbar = tqdm(train_labeled_loader)
    ...
    def kNN(self, K=200, sigma=0.1):    
        ...
        if self.args.dataset == 'imblancestl10':
            trainLabels = torch.LongTensor(self.train_loader.dataset.labeled_data_labels).cuda()
        else:
...
def main():
    ...
    parser.add_argument("--net", type=str, default="wideresnet282",
                        ...
                        help="net name")
    parser.add_argument("--dataset", type=str, default="cifar10", choices=["cifar10", "cifar100", "miniimagenet", "imblancecifar10", "imblancecifar100", "imblancestl10"])
