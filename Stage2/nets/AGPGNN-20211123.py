...
class Trainer(object):
    def __init__(self, X, labels, labeled_idx, classes, nodes_num):
        ...
        model = GPRGNN(K=10, input_dim=128, d_model=1024, output_dim=classes, nodes_num=nodes_num, Wn=Wn)
    ...
    def _train(self, epoch):
        ...
        for i in tbar:
            ...
            ul2 = ul * ul
            uu2 = uu * uu
            Su = torch.diag(uu2).cuda()
            Sl = torch.diag(ul2).cuda()
            Su_inv = torch.diag(1./uu2).cuda()
            uul = uu - ul
            loss_dist = (torch.log(torch.det(Su)/torch.det(Sl)) - Sl.shape[1] + (Su_inv@Sl).trace() + uul.T @ Su_inv @ uul) * 0.5
