...
class Trainer(object):
    def __init__(self, args):
        ...
        if args.net == "resnet18":
            ...
            if self.args.diffuse:
                model = WRN28_2(num_classes=128, diffuse=self.args.diffuse)
            else:
                model = WRN28_2(num_classes=self.args.num_class, diffuse=self.args.diffuse)
        else:
    ...
    def train(self, epoch):
        ...
        for i, sample in enumerate(tbar):
            image, target, ids, image2 = sample["image"], sample["target"], sample["index"], sample["image2"]
            ...
            outputs_2 = self.model(image2)

            loss_mse = mse_loss(outputs, outputs_2)

            # print('outputs, target', outputs.shape, target.shape)
            
            loss = multi_class_loss(outputs, target)
            loss = torch.mean(loss) + 0.1 * loss_mse
    ...
    def label_propagation(self, indicies, diffuse=False):
        ...
        print('Normalize the features + PCA whitening')
        ...
        if diffuse: #Diffusion
            ...
            p_labels, _, _ = diffusion(features, labels.clone(), indicies, k=50, max_iter=50, classes=self.args.num_class)
        ...
        del features
        torch.cuda.empty_cache()
        print('label_propagation finished.')
        return p_labels
...
def main():
    ...
    parser.add_argument("--dataset", type=str, default="cifar10", choices=["cifar10", "cifar100", "miniimagenet", "imblancecifar10", "imblancecifar100", "imblancestl10", "smallimagenet127_x32"])
    ...
    parser.add_argument("--boot-spc", default=0, type=int, help="Number of samples to bootstrap for the augmented labeled set, default: 50 (cifar10) for cifar100 and miniImageNet choose 40")
    ...
    dict_class = {"cifar10":10, "imblancecifar10":10, "imblancecifar100":100, "cifar100":100, "miniimagenet":100, 'imblancestl10':10, 'smallimagenet127_x32':127}
    ...
    spcs = _trainer.train_loader.dataset.img_num_list
    
    for i in ind:
        if len(classes[targ[i]]) < math.ceil(spcs[targ[i]] * args.beta):
            classes[targ[i]].append(targ[i])
            indicies[targ[i]].append(i)
    indicies_new = []
    for ind in indicies:
        for i in ind:
            indicies_new.append(i)
    indicies = indicies_new
    print('Total Labeled samples:', len(indicies), sum(spcs))
    ...
    print('relabel.clone', relabel.shape)
    ...
    ids = ids.view(-1).numpy() #adding the seed samples as having the lowest possible loss
    
    #Order per class
    indexes = np.unique(ids, return_index=True)[1]
    ids = np.array([ids[index] for index in sorted(indexes)]).astype(int)

    print('relabel', relabel.shape)
    relabel_am = torch.argmax(relabel, dim=1) #diffusion labels
    print('relabel_am', relabel_am.shape)
    labeled_num = np.zeros(args.num_class)
    for c in range(args.num_class):
        labeled_num[c] = np.sum(relabel_am.numpy() == c)
    print('labeled_num', labeled_num)

    #Gather args.boot_spc bootstrapped samples per class for the extended labeled set
    boot_spcs = np.zeros(args.num_class).astype(int)
    for idx, img_num in enumerate(_trainer.train_loader.dataset.img_num_list):
        boot_spcs[idx] = min(args.boot_spc, labeled_num[idx])
        boot_spcs[idx] = args.boot_spc
        # boot_spcs[idx] = min(spcs[idx] * 0.5 * 0.1, labeled_num[idx])
    print('boot_spcs', boot_spcs)

    idnx = np.zeros(args.num_class).astype(int)
    ids_class = np.ones((args.num_class,args.boot_spc+_trainer.train_loader.dataset.img_num_list[0])) * (-1)
    # ids_class = np.ones((args.num_class,max(boot_spcs)+_trainer.train_loader.dataset.img_num_list[0])) * (-1)
    print('ids_class', ids_class.shape)
    ...
    for i, ii in enumerate(ids): #sorted by lowest loss, small loss trick, get bottom args.boot_spc samples per class
        ...
        # if idnx[c] < boot_spcs[c]:
            ids_class[c][idnx[c]] = ii
            idnx[c] += 1
    idx_tmp = ids_class.flatten().astype(int)
    print('indicies', indicies.shape)
    idnx_new = np.zeros(args.num_class).astype(int)
    print('per_class_num', idnx, idnx_new)
    for i, ii in enumerate(indicies.numpy().astype(int)):
        #c = relabel_am[ii]
        c = ori_labels[ii]
        if ii in ids_class:
            continue
        ...
        idnx[c] += 1
        idnx_new[c] += 1
    ids_class = ids_class.flatten().astype(int)
    ids_class = ids_class[ids_class >= 0]
    print('ids_class__________________', ids_class.shape)
    ids_class_new = np.unique(ids_class)
    print('ids_class__________________', ids_class_new.shape)
    ...
    print('per_class_acc_num', per_class_acc)
    print('per_class_num', idnx, idnx_new)
    print("Per class acc : ", per_class_acc / idnx)
    print("Mean class acc : ", torch.mean(per_class_acc / idnx))
    print("STD class acc : ", torch.std(per_class_acc / idnx))
    
    np.savez(os.path.join(args.save_dir, "labels_seed{}_beta_{}".format(args.seed, beta)), relabel.numpy())
    np.savez(os.path.join(args.save_dir, "subset_seed{}_beta_{}".format(args.seed, beta)), ids_class)
