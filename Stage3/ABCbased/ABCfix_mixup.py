...
parser.add_argument('labels', type=str, default='./imbcifar10_imbfactor_100/seed1/labels_seed1_beta_10.0.npz')
parser.add_argument('labeled', type=str, default='./imbcifar10_imbfactor_100/seed1/subset_seed1_beta_10.0.npz')
...
def newmixup(inputs, tartgets):
    global N_label, N_unlabel, Z, z
    # N = torch.tensor([500, 299, 179, 107, 64, 38, 23, 13, 8, 5], device=N_unlabel.device)
    N = torch.tensor([1500, 899, 539, 323, 193, 116, 69, 41, 25, 15], device=inputs.device)
    N = torch.flip(-torch.log(torch.abs(N / torch.sum(N)) + 1e-12), dims=[0])

    Na = N[torch.argmax(tartgets, dim=1)]

    idx = torch.randperm(inputs.size(0))

    input_a, input_b = inputs, inputs[idx]
    target_a, target_b = tartgets, tartgets[idx]
    N_a, N_b = Na, Na[idx]
    ...
    lam = lam_base.repeat(inputs.shape[0])
    lam = torch.from_numpy(lam).to(device=tartgets.device)

    lam = torch.where(lam>0.5, lam, 1-lam)

    mixed_input = lam.reshape(lam.shape[0],1,1,1) * input_a + (1 - lam.reshape(lam.shape[0],1,1,1)) * input_b

    lam = torch.where(N_a<N_b, lam, 1.-lam)
    lam = rho * lam
    lam = torch.where(lam>1., torch.ones_like(lam), lam)
    lam = torch.where(N_a<N_b, lam, 1-lam)
    lam = lam.reshape(lam.shape[0], 1)
    mixed_target = lam * target_a + (1. - lam) * target_b

    return mixed_input, mixed_target.detach()
...
def mixup_data(x):
    ...
    if lam > 0.5:
        lam = 1 - lam 
    batch_size = x.size()[0]
    index = torch.randperm(batch_size).to(device=x.device)
...
def train(labeled_trainloader, unlabeled_trainloader, model, optimizer, ema_optimizer, criterion, epoch,ir2):
    ...
    for batch_idx in range(args.val_iteration):
        ...
        # mixup
        inputs_x_mix, targets_x2_mix = newmixup(inputs_x, targets_x2)
        # inputs_u, inputs_u2, inputs_u3 = mixup_data(inputs_u), mixup_data(inputs_u2), mixup_data(inputs_u3)
        ...
        # mixedup
        # inputs_u2, targets_u2_1 = newmixup(inputs_u2, targets_u2)
        # inputs_u3, targets_u2_2 = newmixup(inputs_u3, targets_u2)
        # _, p_hat_u1 = torch.max(targets_u2_1, dim=1)
        # p_hat_u1 = torch.zeros(batch_size, num_class).cuda().scatter_(1, p_hat_u1.view(-1, 1), 1)
        # _, p_hat_u2 = torch.max(targets_u2_2, dim=1)
        # p_hat_u2 = torch.zeros(batch_size, num_class).cuda().scatter_(1, p_hat_u2.view(-1, 1), 1)

        targets_u = torch.argmax(targets_u2, dim=1)

        q = model(inputs_x_mix)
        q2 = model(inputs_u2)
        q3 = model(inputs_u3)
        ...
        all_targets = torch.cat([targets_x2_mix, p_hat, p_hat], dim=0)
        # all_targets = torch.cat([targets_x2, p_hat_u1, p_hat_u2], dim=0)
