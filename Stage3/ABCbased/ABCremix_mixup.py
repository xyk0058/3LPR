...
# ema
parser.add_argument('--ema_beta', type=float, default=0.4, help='ema beta.')

parser.add_argument('--labels', default='./imbcifar10_imbfactor_100/seed1/labels_seed1_beta_30.0.npz', type=str)
parser.add_argument('--labeled', default='./imbcifar10_imbfactor_100/seed1/subset_seed1_beta_30.0.npz', type=str)
parser.add_argument('--select_idx', default='./imbcifar10_imbfactor_100/seed1/select_idx_imb100_cifar10.npz', type=str)
...
if args.dataset=='cifar10':
    ...
    num_class = 100
elif args.dataset=='smallimagenet127_x32':
    import dataset.imbSmallImageNet127 as dataset
    print(f'==> Preparing smallimagenet127_x32')
    num_class = 127
...
N_label, N_unlabel, Z, z = None, None, None, None

def main():
    global N_label, N_unlabel, Z, z
    global best_acc
    ...
    if args.dataset == 'cifar10':
        train_labeled_set, train_unlabeled_set,test_set, train_labeled_idxs, train_unlabeled_idxs = dataset.get_cifar10(args, './data', N_SAMPLES_PER_CLASS,U_SAMPLES_PER_CLASS)
        ...
        train_labeled_set, train_unlabeled_set,test_set, train_labeled_idxs, train_unlabeled_idxs = dataset.get_smallimagenet127(args, './data', N_SAMPLES_PER_CLASS,U_SAMPLES_PER_CLASS)
    ...
    # cnt = torch.zeros(10)
    # for inputs_x, targets_x, idx_l in unlabeled_trainloader:
    #     for i in range(10):
    #         cc = torch.where(targets_x == i, 1, 0)
    #         cnt[i] += torch.sum(cc)
    # print('cnt', cnt)
    ...
    N_label = np.array(N_SAMPLES_PER_CLASS)
    N_unlabel = np.zeros(num_class)
    Z = np.zeros([50000, num_class])
    z = np.zeros([50000, num_class])
    print('train_labeled_set.getAllLabeles()', len(train_labeled_set.getAllLabeles()))
    Z[train_labeled_idxs] = np.eye(num_class)[train_labeled_set.getAllLabeles()]
    z[train_labeled_idxs] = np.eye(num_class)[train_labeled_set.getAllLabeles()]

    N_label = torch.from_numpy(N_label).cuda()
    N_unlabel = torch.from_numpy(N_unlabel).cuda()
...
def train(labeled_trainloader, unlabeled_trainloader, model, optimizer, ema_optimizer, criterion, epoch,ir2, emp_distb_u,target_disb):
    ...
    for batch_idx in range(args.val_iteration):
        try:
            inputs_x, targets_x, idx_l = labeled_train_iter.next()
        except:
            labeled_train_iter = iter(labeled_trainloader)
            inputs_x, targets_x, idx_l = labeled_train_iter.next()
        ...
        # new mixup
        z_old = torch.sum(z[idx_u], axis=0)
        Z[idx_u] = args.ema_beta * Z[idx_u] + (1 - args.ema_beta) * targets_u2
        z[idx_u] = Z[idx_u] * (1. / (1. - args.ema_beta ** (epoch + 1)))
        targets_u2_ = z[idx_u]
        z_new = torch.sum(z[idx_u], axis=0)
        N_unlabel = N_unlabel - z_old + z_new

        # N = N_unlabel + N_label
        # N = torch.flip(-torch.log(torch.abs(N / torch.sum(N)) + 1e-12), dims=[0])
        
        # N = torch.tensor([1500, 899, 539, 323, 193, 116, 69, 41, 25, 15], device=N_unlabel.device)
        N = torch.tensor([500, 299, 179, 107, 64, 38, 23, 13, 8, 5], device=N_unlabel.device)
        N = torch.flip(-torch.log(torch.abs(N / torch.sum(N)) + 1e-12), dims=[0])
        # N = torch.log(N + 1e-12)

        all_inputs = torch.cat([inputs_x, inputs_u, inputs_u2, inputs_u3], dim=0)
        all_targets = torch.cat([targets_x2, targets_u2_, targets_u2_, targets_u2_], dim=0)
        Na = N[torch.argmax(all_targets, dim=1)]
        ...
        tmp = torch.where(N_a<N_b, N_b / N_a, N_a / N_b) - 1
        c_1 = -torch.sum(target_a * torch.log(torch.abs(target_a+1e-12)), dim=1)
        c_2 = -torch.sum(target_b * torch.log(torch.abs(target_b+1e-12)), dim=1)
        # print('N_a', N_a.shape, N_b.shape, c_1.shape, c_2.shape)
        confidence = 1. + torch.where(N_a<N_b, c_1, c_2)
        rho = 1. + confidence * (tmp / (N[0] / N[-1]))
        # lam_l = np.random.beta(args.mix_alpha, args.mix_alpha, [inputs_x.shape[0]]).astype('f')
        lam_base = np.random.beta(args.mix_alpha, args.mix_alpha, [1]).astype('f')
        lam_l = lam_base.repeat(inputs_x.shape[0])
        rho_mean = torch.mean(rho).cpu().item()
        lam_base_u = np.random.beta(rho_mean, 1, [1]).astype('f')
        lam_u = lam_base_u.repeat(inputs_u.shape[0]+inputs_u2.shape[0]+inputs_u3.shape[0])
        
        lam_l = torch.from_numpy(lam_l).to(device=all_targets.device)
        lam_u = torch.from_numpy(lam_u).to(device=all_targets.device)

        lam_l = torch.where(lam_l>0.5, lam_l, 1-lam_l)

        lam_ = torch.cat([lam_l, lam_u], dim=0)
        mixed_input = lam_.reshape(lam_.shape[0],1,1,1) * input_a + (1 - lam_.reshape(lam_.shape[0],1,1,1)) * input_b
        lam_lra_l = lam_[:inputs_x.shape[0]].reshape(inputs_x.shape[0],1)
        mixed_target_l = lam_lra_l * target_a[:inputs_x.shape[0]] + (1 - lam_lra_l) * target_b[:inputs_x.shape[0]]

        lam = torch.where(N_a<N_b, lam_, 1.-lam_)
        lam_lra_u = rho * lam
        lam_lra_u = torch.where(lam_lra_u>1., torch.ones_like(lam_lra_u), lam_lra_u)
        lam_lra_u = torch.where(N_a<N_b, lam_lra_u, 1-lam_lra_u)
        lam_lra_u = lam_lra_u[inputs_x.shape[0]:]
        lam_lra_u = lam_lra_u.reshape(lam_lra_u.shape[0],1)
        mixed_target_u = lam_lra_u * target_a[inputs_x.shape[0]:] + (1. - lam_lra_u) * target_b[inputs_x.shape[0]:]
        mixed_target = torch.cat([mixed_target_l, mixed_target_u], dim=0).contiguous()
        # print('mixedinput', type(mixed_input), type(mixed_target))
        # breaks
