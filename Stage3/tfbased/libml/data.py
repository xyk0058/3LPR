...
def record_parse(serialized_example, image_shape=None):
    ...
    # image_data = tf.image.decode_png(features['image'])
    # image = tf.image.resize_images(image_data, image_shape[:2])
    ...
    image = tf.cast(image, tf.float32) * (2.0 / 255) - 1.0
    return dict(image=image, label=features['label'], index=features['index'])
...
class DataSet:
    ...
    def from_files(cls, filenames: list, augment_fn: AugmentPair, parse_fn=record_parse, image_shape=None):
        ...
        print('filenames_from_files', filenames)
...
class DataSets:
    def __init__(self, name, train_labeled: DataSet, train_unlabeled: DataSet, test: DataSet, valid: DataSet,
                 height=32, width=32, colors=3, nclass=10, mean=0, std=1, p_labeled=None, p_unlabeled=None, unlabeled_num=None, labeled_num=None, test_num=None, N=None, labeled_index=None, labels=None):
        self.name = name
        ...
        self.N = N      # N indicate the amount of samples in class
        self.labeled_index = labeled_index
        self.labels = labels

    def countTFRecord(dataset_path, nclass=None):
        count = 0
        if not nclass is None:
            N = np.zeros(nclass)
        mmin = 1000000000
        mmax = 0
        index_list = []
        labels = []
        for record in tf.python_io.tf_record_iterator(dataset_path):
            count += 1
            if nclass is None:
                continue
            s = tf.train.Example.FromString(record)
            f = dict(s.features.feature)
            c = s.features.feature['label'].int64_list.value[0]
            N[c] += 1
            idx = s.features.feature['index'].int64_list.value[0]
            mmin = min(idx, mmin)
            mmax = max(idx, mmax)
            index_list.append(idx)
            labels.append(c)
        # print('dataset[', dataset_path, ']', mmin, mmax)
        if nclass is None:
            return count
        else:
            return count, N, np.array(index_list), np.array(labels)

    @classmethod
    def creator(cls, name, seed=None, label=None, valid=1, augment=augment_module.many_augment_function, parse_fn=record_parse, do_memoize=False,
                nclass=10, colors=3, height=32, width=32):
        #fullname = '.%d@%d' % (seed, label)
        print('name!!!!!!!!!!!!!!!!!!', name, height, width)
        datasetn = name.split('.')[0]
        fullname = name[len(datasetn):]
        name = datasetn
        ...
        def create():
            ...
            print('image_shape0!!!!!!!!!!!!!!!!!!!!!!!', image_shape)
            train_labeled = DataSet.from_files(
                [root + fullname + '-label.tfrecord'], augment[0], parse_fn, image_shape)
            labeled_num, N, labeled_index, labels = cls.countTFRecord(root + fullname + '-label.tfrecord', nclass)
            print('train_labeled-DATASET', root + fullname + '-label.tfrecord', labeled_num)

            train_unlabeled = DataSet.from_files(
                [root + '-unlabel.tfrecord'], augment[1], parse_fn, image_shape)
            # unlabeled_num = cls.countTFRecord(root + '-unlabel.tfrecord')
            # print('train_unlabeled-DATASET', root + '-unlabel.tfrecord', unlabeled_num)
            unlabeled_num, N_, unlabeled_index, unlabels = cls.countTFRecord(root + '-unlabel.tfrecord', nclass)
            print('train_unlabeled-DATASET', root + '-unlabel.tfrecord', unlabeled_num)
            ...
            test_num = cls.countTFRecord(os.path.join(DATA_DIR, '%s-test.tfrecord' % name))
            print('TEST-DATASET', os.path.join(DATA_DIR, '%s-test.tfrecord' % name), test_num)
            ...
            return cls(name + '.' + FLAGS.augment + fullname + '-' + str(valid)
                       ...
                       unlabeled_num=unlabeled_num, labeled_num=labeled_num, test_num=test_num, N=N,
                       labeled_index=labeled_index, labels=labels)
...
def create_datasets(augment_fn):
    ...
    d.update([DataSets.creator('cifar100', seed, label, valid, augment_fn, nclass=100)
              for seed, label, valid in itertools.product(range(6), [2500, 10000], [1, 5000])])
