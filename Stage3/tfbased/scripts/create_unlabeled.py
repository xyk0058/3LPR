...
def main(argv):
    ...
    print('imb_select_idx', select_idx.shape)
    ori_label = dict()
            ...
            for record in tf.python_io.tf_record_iterator(input_file):
                ...
                while True:
                    ...
                    if class_data[c]:
                        ...
                        s = tf.train.Example.FromString(v)
                        f = dict(s.features.feature)
                        f['index'] = tf.train.Feature(int64_list=tf.train.Int64List(value=[ori_label[p]]))
                        s = tf.train.Example(features=tf.train.Features(feature=f))
                        v = s.SerializeToString()
                        mmax = max(mmax, ori_label[p])
    ...
    print('unlabel', len(unlabel), 'maxindex', mmax)
