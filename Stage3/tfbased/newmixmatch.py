...
class NewMixMatch(models.MultiModel):
    ...
    def model(self, batch, lr, wd, ema, beta, w_match, warmup_kimg=1024, nu=2, mixmode='xxy.yxy', dbuf=128, **kwargs):
        ...
        Number = tf.placeholder(tf.float32, [self.nclass], 'Number')
        ...
        Z_u = tf.tile(Z_unlabeled, [nu, 1])
        xy, labels_xy, l_dbg = augment([xt_in] + tf.split(y, nu), [lx] + [ly] * nu, [beta, beta], 
                            N=[Number, FLAGS.imb_fac, Z_u, Z_labeled, FLAGS.ema_beta, sample_epoch])
        x, y = xy[0], xy[1:]
        ...
        return EasyDict(
            ...
            l_dbg=l_dbg+[w_match], ly=ly,
            Number=Number, Z_unlabeled=Z_unlabeled, Z_labeled=Z_labeled, sample_epoch=sample_epoch)
