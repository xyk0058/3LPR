...
class ABCNewMatch(MixMatch):
    ...
    def model(self, batch, lr, wd, beta, w_kl, w_match, w_rot, K, use_xe, warmup_kimg=1024, T=0.5,
        ...
        def np_bernoulli(n, p):
            return np.random.binomial([1 for _ in range(n)], p).astype('f')

        print('model', self.dataset.unlabeled_num, self.dataset.labeled_num, self.dataset.test_num, K)
        hwc = [self.dataset.height, self.dataset.width, self.dataset.colors]
        xt_in = tf.placeholder(tf.float32, [batch] + hwc, 'xt')  # For training
        x_in = tf.placeholder(tf.float32, [None] + hwc, 'x')
        y_in = tf.placeholder(tf.float32, [batch, K + 1] + hwc, 'y')
        l_in = tf.placeholder(tf.int32, [batch], 'labels')
        N = tf.placeholder(tf.float32, [self.nclass], 'N')
        Z_unlabeled = tf.placeholder(tf.float32, [batch, self.nclass], 'Z_unlabeled')
        Z_labeled = tf.placeholder(tf.float32, [batch, self.nclass], 'Z_labeled')
        labeled_idx = tf.placeholder(tf.int32, [batch], 'labeled_idx')
        labeled_label = tf.placeholder(tf.float32, [batch, self.nclass], 'labeled_label')
        sample_epoch = tf.placeholder(tf.float32, [batch, 1], 'sample_epoch')
        ir2 = tf.placeholder(tf.float32, [self.nclass], 'ir2')

        # lrate = tf.clip_by_value(tf.to_float(self.step) / (FLAGS.train_kimg << 10), 0, 1)
        # lr *= tf.cos(lrate * (7 * np.pi) / (2 * 8))
        # tf.summary.scalar('monitors/lr', lr)

        base_prob = tf.math.log((N / tf.reduce_sum(N))**0.5+1e-12)

        indexes_unlabeled = tf.placeholder(tf.int32, [batch], 'indexes_unlabeled')
        wd *= lr
        ...
        def classifier2_to_gpu(x, **kw):
            with tf.device(next(gpu)):
                return self.classifier(x, **kw, **kwargs).logits2
        ...
        # ema_y = layers.ExponentialMovingAverage('ema_y', self.nclass, )
        ...
        # N_batch = tf.reduce_mean(guess.p_target, axis=0)
        # loss_e = -tf.reduce_sum(N_prob * tf.log(N_batch), axis=0)
        ...
        Z_u = tf.tile(Z_unlabeled, [K + 1, 1])
        xy, labels_xy, l_dbg = augment([xt_in] + [y_in[:, i] for i in range(K + 1)], [lx] + tf.split(ly, K + 1),
                                [beta, beta], N=[N, FLAGS.imb_fac, Z_u, Z_labeled, FLAGS.ema_beta, sample_epoch, labeled_idx, labeled_label])
        x, y = xy[0], xy[1:]
        labels_x, labels_y = labels_xy[0], tf.concat(labels_xy[1:], 0)
        labels_x_ = tf.stop_gradient(labels_x)
        labels_y_ = tf.stop_gradient(labels_y)
        del xy, labels_xy, Z_u, labels_x, labels_y

        # abc
        logit = tf.nn.softmax(classifier2_to_gpu(xt_in, training=True))
        logitsu = [tf.nn.softmax(classifier2_to_gpu(y_in[:, i], training=True)) for i in range(K + 1)]
        logitsu1, logitsu2, logitsu3 = logitsu[0], logitsu[1], logitsu[2]
        max_p = tf.reduce_max(logitsu1, axis=1)
        label_u = tf.argmax(logitsu1, axis=1)
        
        epoch = tf.cast(self.step, tf.float32) // (FLAGS.report_kimg << 10)
        ir22 = 1 - (epoch / (1 << 9)) * (1 - ir2)
        with tf.device('/cpu'):
            p = tf.gather(ir2, tf.argmax(labeled_label, 1))
            maskforbalance = tf.stop_gradient(tf.py_func(np_bernoulli, [batch, p], tf.float32))
            p2 = tf.gather(ir22, label_u)
            maskforbalanceu = tf.stop_gradient(tf.py_func(np_bernoulli, [batch, p2], tf.float32))

        select_mask = tf.stop_gradient(tf.where(max_p>0.95, tf.ones_like(max_p), tf.zeros_like(max_p)))
        abcloss = -tf.reduce_mean(maskforbalance * tf.reduce_sum(tf.log(logit) * labeled_label, axis=1))
        abcloss1 = -tf.reduce_mean(select_mask * maskforbalanceu  * tf.reduce_sum(tf.log(logitsu2) * tf.stop_gradient(logitsu1), axis=1))
        abcloss2 = -tf.reduce_mean(select_mask * maskforbalanceu  * tf.reduce_sum(tf.log(logitsu3) * tf.stop_gradient(logitsu1), axis=1))
        ...
        del batches, logits
        pred = tf.nn.softmax(logits_x)
        loss_xe = -tf.reduce_sum(labels_x_ * tf.log(pred),reduction_indices = 1)
        #loss_xe = tf.nn.softmax_cross_entropy_with_logits_v2(labels=labels_x_, logits=logits_x+base_prob)
        loss_xe = tf.reduce_mean(loss_xe)
        if use_xe:
            pred = tf.nn.softmax(logits_y)
            loss_xeu = -tf.reduce_sum(labels_y_ * tf.log(pred),reduction_indices = 1)
            #loss_xeu = tf.nn.softmax_cross_entropy_with_logits_v2(labels=labels_y_, logits=logits_y)
        else:
        ...
        # all_loss = loss_xe + w_kl * loss_kl + w_match * loss_xeu + w_rot * loss_rot

        train_op = tf.train.AdamOptimizer(lr).minimize(loss_xe + totalabcloss
                                                       + w_kl * loss_kl
                                                       + w_match * loss_xeu
                                                       + w_rot * loss_rot,
                                                       colocate_gradients_with_ops=True)
        # train_op = tf.train.MomentumOptimizer(lr, 0.9, use_nesterov=True).minimize(loss_xe
        #                                                + w_kl * loss_kl
        #                                                + w_match * loss_xeu
        #                                                + w_rot * loss_rot,
        #                                                colocate_gradients_with_ops=True)
        ...
        return EasyDict(
            ...
            ly=ly, N=N, l_dbg=l_dbg+[ir22, epoch, abcloss, abcloss1, abcloss2], Z_unlabeled=Z_unlabeled, Z_labeled=Z_labeled, sample_epoch=sample_epoch,
            labeled_idx=labeled_idx, labeled_label=labeled_label, ir2=ir2)
